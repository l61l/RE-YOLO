...
from .metrics import bbox_iou,bbox_iouim
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        # EIoU
        # iou = bbox_iouim(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, SIoU=True,Focal=True)
        # if type(iou) is tuple:
        ...
        if self.use_dfl:
            ...
            loss_dfl = self._df_loss(
                pred_dist[fg_mask].view(-1, self.reg_max + 1), target_ltrb[fg_mask]) * weight
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(
            m.reg_max - 1, use_dfl=self.use_dfl).to(device)
...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        ...
        # Cls loss
        # loss[1] = self.varifocal_loss(pred_scores, target_scores, target_labels) / target_scores_sum  # VFL way
        loss[2] = self.bce(pred_scores, target_scores.to(
            dtype)).sum() / target_scores_sum  # BCE
    ...
    def calculate_segmentation_loss(
        ...
        mxyxy = target_bboxes_normalized * \
            torch.tensor([mask_w, mask_h, mask_w, mask_h], device=proto.device)
...
class v8PoseLoss(v8DetectionLoss):
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        sigmas = torch.from_numpy(OKS_SIGMA).to(
            self.device) if is_pose else torch.ones(nkpt, device=self.device) / nkpt
        self.keypoint_loss = KeypointLoss(sigmas=sigmas)

    def __call__(self, preds, batch):
        """Calculate the total loss and detach it."""
        loss = torch.zeros(
            5, device=self.device)  # box, cls, dfl, kpt_location, kpt_visibility
        ...
        pred_bboxes = self.bbox_decode(
            anchor_points, pred_distri)  # xyxy, (b, h*w, 4)
        pred_kpts = self.kpts_decode(anchor_points, pred_kpts.view(
            batch_size, -1, *self.kpt_shape))  # (b, h*w, 17, 3)
        ...
        # Cls loss
        # loss[1] = self.varifocal_loss(pred_scores, target_scores, target_labels) / target_scores_sum  # VFL way
        loss[3] = self.bce(pred_scores, target_scores.to(
            dtype)).sum() / target_scores_sum  # BCE
    ...
    def calculate_keypoints_loss(self, masks, target_gt_idx, keypoints, batch_idx, stride_tensor, target_bboxes,
        ...
        if masks.any():
            ...
            kpt_mask = gt_kpt[..., 2] != 0 if gt_kpt.shape[-1] == 3 else torch.full_like(
                gt_kpt[..., 0], True)
            kpts_loss = self.keypoint_loss(
                pred_kpt, gt_kpt, kpt_mask, area)  # pose loss

            if pred_kpt.shape[-1] == 3:
                kpts_obj_loss = self.bce_pose(
                    pred_kpt[..., 2], kpt_mask.float())  # keypoint obj loss
...
class v8ClassificationLoss:
    ...
    def __call__(self, preds, batch):
        ...
        loss = torch.nn.functional.cross_entropy(
            preds, batch['cls'], reduction='sum') / 64
