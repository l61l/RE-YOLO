...
def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
    ...
    if fg_mask.max() > 1:  # one anchor is assigned to multiple gt_bboxes
        mask_multi_gts = (fg_mask.unsqueeze(1) > 1).expand(-1,
                                                           n_max_boxes, -1)  # (b, n_max_boxes, h*w)
...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        if self.n_max_boxes == 0:
            ...
            return (torch.full_like(pd_scores[..., 0], self.bg_idx).to(device), torch.zeros_like(pd_bboxes).to(device),
                    torch.zeros_like(pd_scores).to(device), torch.zeros_like(
                        pd_scores[..., 0]).to(device),
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        ind[0] = torch.arange(end=self.bs).view(-1,
                                                1).expand(-1, self.n_max_boxes)  # b, max_num_obj
        ind[1] = gt_labels.squeeze(-1)  # b, max_num_obj
        # Get the scores of each grid for each gt cls
        bbox_scores[mask_gt] = pd_scores[ind[0], :,
                                         ind[1]][mask_gt]  # b, max_num_obj, h*w

        # (b, max_num_obj, 1, 4), (b, 1, h*w, 4)
        pd_boxes = pd_bboxes.unsqueeze(
            1).expand(-1, self.n_max_boxes, -1, -1)[mask_gt]
...
#修改loss
        ...
        # overlaps[mask_gt] = bbox_iouim(gt_boxes, pd_boxes, xywh=False, SIoU=True).squeeze(-1).clamp_(0)
    ...
    def select_topk_candidates(self, metrics, largest=True, topk_mask=None):
        ...
        if topk_mask is None:
            topk_mask = (topk_metrics.max(-1, keepdim=True)
                         [0] > self.eps).expand_as(topk_idxs)
        # (b, max_num_obj, topk)
...
def make_anchors(feats, strides, grid_cell_offset=0.5):
    ...
    for i, stride in enumerate(strides):
        ...
        sx = torch.arange(end=w, device=device, dtype=dtype) + \
            grid_cell_offset  # shift x
        sy = torch.arange(end=h, device=device, dtype=dtype) + \
            grid_cell_offset  # shift y
