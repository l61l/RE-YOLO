...
OKS_SIGMA = np.array([.26, .25, .25, .35, .35, .79, .79, .72, .72, .62, .62,
...
def bbox_ioa(box1, box2, iou=False, eps=1e-7):
    ...
    inter_area = (np.minimum(b1_x2[:, None], b2_x2) - np.maximum(b1_x1[:, None], b2_x1)).clip(0) * \
                 (np.minimum(b1_y2[:, None], b2_y2) -
                  np.maximum(b1_y1[:, None], b2_y1)).clip(0)
...
def bbox_iouim(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, Focal=False, alpha=1, gamma=0.5, eps=1e-7):
    # Returns Intersection over Union (IoU) of box1(1,4) to box2(n,4)
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            rho2 = (((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 +
                    b2_y2 - b1_y1 - b1_y2) ** 2) / 4) ** alpha  # center dist ** 2
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * \
                ...
                angle_cost = torch.cos(
                    torch.arcsin(sin_alpha) * 2 - math.pi / 2)
                rho_x = (s_cw / cw) ** 2
                rho_y = (s_ch / ch) ** 2
                gamma = angle_cost - 2
                distance_cost = 2 - \
                    torch.exp(gamma * rho_x) - torch.exp(gamma * rho_y)
                omiga_w = torch.abs(w1 - w2) / torch.max(w1, w2)
                omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
                shape_cost = torch.pow(
                    1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                if Focal:
                    # Focal_SIou
...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.45, task='detect'):
        ...
        self.matrix = np.zeros(
            (nc + 1, nc + 1)) if self.task == 'detect' else np.zeros((nc, nc))
...
def ap_per_class(tp,
    ...
    ap, p_curve, r_curve = np.zeros((nc, tp.shape[1])), np.zeros(
    ...
    if plot:
        ...
        plot_mc_curve(x, f1_curve, save_dir /
                      f'{prefix}F1_curve.png', names, ylabel='F1', on_plot=on_plot)
        plot_mc_curve(x, p_curve, save_dir /
                      f'{prefix}P_curve.png', names, ylabel='Precision', on_plot=on_plot)
        plot_mc_curve(x, r_curve, save_dir /
                      f'{prefix}R_curve.png', names, ylabel='Recall', on_plot=on_plot)
