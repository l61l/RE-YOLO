...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    RTDETRDecoder, Segment, RFAConv,RFCBAMConv, ASPP, ASFF_2, ASFF_3, MHSA, SimSPPF, EMA_Bottleneck, C2f_EAM)
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, (Detect, Segment, Pose)):
            ...
            def forward(x): return self.forward(x)[0] if isinstance(
                m, (Segment, Pose)) else self.forward(x)
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta = preds if self.training else preds[
            1]
        if dn_meta is None:
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    ensemble.stride = ensemble[torch.argmax(torch.tensor(
        [m.stride.max() for m in ensemble])).int()].stride
    assert all(
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 RFAConv, RFCBAMConv,ASPP, SimSPPF, EMA_Bottleneck, C2f_EAM):
            c1, c2 = ch[f], args[0]
